...
static int mpc_parse_run(mpc_input_t *i, mpc_parser_t *p, mpc_result_t *r,
    ...
    switch (p->type) {
        ...
        case MPC_TYPE_SINGLE:
            MPC_PRIMITIVE(
                mpc_input_char(i, p->data.single.x, (char **)&r->output));
        case MPC_TYPE_RANGE:
            MPC_PRIMITIVE(mpc_input_range(i, p->data.range.x, p->data.range.y,
                                          (char **)&r->output));
        case MPC_TYPE_ONEOF:
            MPC_PRIMITIVE(
                mpc_input_oneof(i, p->data.string.x, (char **)&r->output));
        ...
        case MPC_TYPE_MANY:
            ...
            while (
                mpc_parse_run(i, p->data.repeat.x, &results[j], e, depth + 1)) {
                j++;
        ...
        case MPC_TYPE_MANY1:
            ...
            if (j == 0) {
                ...
                *e = mpc_err_merge(i, *e, results[j].error);
                ...
                MPC_SUCCESS(
                    mpc_parse_fold(i, p->data.repeat.f, j,
                                   (mpc_val_t **)results);
                    if (j >= MPC_PARSE_STACK_MIN) { mpc_free(i, results); });
            }
        ...
        case MPC_TYPE_SEPBY1:
            ...
            if (mpc_parse_run(i, p->data.sepby1.x, &results[j], e, depth + 1)) {
                ...
                while (mpc_parse_run(i, p->data.sepby1.sep, &results[j], e,
                                     depth + 1) &&
                       mpc_parse_run(i, p->data.sepby1.x, &results[j], e,
                                     depth + 1)) {
                    j++;
                    results = mpc_grow_results(i, j, results_stk, results);
                }
        ...
        case MPC_TYPE_COUNT:
            ...
            results =
                p->data.repeat.n > MPC_PARSE_STACK_MIN
            ...
            if (j == p->data.repeat.n) {
                MPC_SUCCESS(mpc_parse_fold(i, p->data.repeat.f, j,
                                           (mpc_val_t **)results);
        ...
        case MPC_TYPE_AND:
            ...
            for (j = 0; j < p->data.and.n; j++) {
                if (!mpc_parse_run(i, p->data.and.xs[j], &results[j], e,
                    ...
                    mpc_input_rewind(i);
                    for (k = 0; k < j; k++) {
                        mpc_parse_dtor(i, p->data.and.dxs[k],
                                       results[k].output);
                    }
...
mpc_parser_t *mpc_copy(mpc_parser_t *a) {
    ...
    switch (a->type) {
        ...
        case MPC_TYPE_AND:
            ...
            if (a->data.and.n > 0) {
                p->data.and.dxs =
                    malloc((a->data.and.n - 1) * sizeof(mpc_dtor_t));
    ...
    switch (c) {
        ...
        case 'A':
            return mpc_and(2, mpcf_snd, mpc_soi(), mpc_lift(mpcf_ctor_str),
                           free);
        case 'Z':
            return mpc_and(2, mpcf_snd, mpc_eoi(), mpc_lift(mpcf_ctor_str),
                           free);
        case 'd':
            return mpc_digit();
        case 'D':
...
static const char *mpc_re_range_escape_char(char c) {
    switch (c) {
        ...
        case 's':
            return " \f\n\r\t\v";
        case 'w':
            return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456"
                   "789_";
        default:
            return NULL;
    }
...
static void mpc_optimise_unretained(mpc_parser_t *p, int force) {
    ...
    while (1) {
        ...
        /* Merge lhs `or` */
        if (p->type == MPC_TYPE_OR && p->data.or.xs[0]->type == MPC_TYPE_OR &&
