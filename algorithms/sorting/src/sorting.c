...
// Worst-case complexity:  O(n) total O(1) auxillary
int *heap_sort(int array[], int heap_size) {
...
int *heapify(int array[], int heap_size, int root) {
    int left = 2 * root + 1;
    int right = 2 * root + 2;
    int largest = root;
...
void merge_sort_r(int array[], int begin, int end) {
    if (begin >= end)
        return;
...
// Best-case performance:       O(n^2) comparisons (O(n) with best-case check),
// O(1) swap Average performance:         O(n^2) comparisons, O(n) swaps
...
int is_best_case_array(int array[], int array_length) {
    int best_case = 1, i;
