...
int rbt_delete_node(rbt *tree, rbt_node *v) {
    rbt_node *parent = v->parent, *u = _rbt_bst_replace(v);
...
int rbt_delete_node_by_key(rbt *tree, int key) {
...
rbt_node *rbt_insert(rbt *tree, int key) {
...
void rbt_print_inorder(rbt *tree) {
    _rbt_print_inorder_helper(tree, tree->root);
}

rbt_node *rbt_search(rbt *tree, int key) {
    return _rbt_search_helper(tree->root, key);
}

// "private" functions

rbt_node *_rbt_bst_replace(rbt_node *node) {
...
rbt_node *_rbt_create_node(int key) {
...
void _rbt_fix_double_black(rbt *tree, rbt_node *n) {
    if (n == tree->root) {
        // reached root
        return;
    }

    rbt_node *parent = n->parent, *sibling = _rbt_get_sibling(n);
...
void _rbt_free_helper(rbt_node *node) {
    if (node == NIL) {
        return;
    }
...
rbt_node *_rbt_get_sibling(rbt_node *u) {
    if (u == NIL) {
        return NIL;
    }
...
void _rbt_insert_fix(rbt *tree, rbt_node *node) {
...
boolean _rbt_node_has_red_child(rbt_node *n) {
    if (n == NIL) {
        return FALSE;
    }
...
boolean _rbt_node_is_on_parent_left(rbt_node *n) {
    if (n->parent == NIL) {
        return FALSE;
    }
...
void _rbt_print_inorder_helper(rbt *tree, rbt_node *node) {
    if (node == NIL) {
        return;
    }
...
void _rbt_rotate_left(rbt *tree, rbt_node *x) {
    rbt_node *y = x->right;
...
void _rbt_rotate_right(rbt *tree, rbt_node *x) {
    rbt_node *y = x->left;
...
rbt_node *_rbt_search_helper(rbt_node *node, int key) {
...
rbt_node *_rbt_successor(rbt_node *node) {
...
void _rbt_swap_node_values(rbt_node *u, rbt_node *v) {
    int temp = u->key;
    u->key = v->key;
    v->key = temp;
}
